...
hook_event(HOOK_BEFORE_PHYS_STEP, function(m)
    if not ou64_flood_active then
        ...
        if m.area.index ~= 1 and
                floor ~= nil then
            for i, entry in ipairs(ou64_areas) do
...
hook_event(HOOK_UPDATE, function()
    ...
    if is_game_paused() then
        audio_stream_set_volume(ou64_background_music, ou64_music_volume / 10)
    else
        audio_stream_set_volume(ou64_background_music, ou64_music_volume)
    end

    -- Mod Sort Hack (So Only Up 64 shows first)
    if true and false then
        djui_chat_message_create("ExtraDataExtraDataExtraDataExtraDataExtraDataExtraDataExtraDataExtraData\
